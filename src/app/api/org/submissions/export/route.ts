...
export async function GET(request: NextRequest) {
  ...
  if (!profile?.is_active || !["org_admin", "org_member"].includes(profile.role ?? "")) {
  ...
  const header = ["id", "url", "domain", "report_count", "categories", "descriptors", "first_reported_at", "last_reported_at"];
  const csv = [header, ...rows].map((r) => r.map(csvCell).join(",")).join("\r\n");

  return new NextResponse(csv, {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": `attachment; filename="harm-watch-feed-${new Date().toISOString().slice(0, 10)}.csv"`,
    },
  });
}
