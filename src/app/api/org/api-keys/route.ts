...
async function getOrgAdmin(supabase: Awaited<ReturnType<typeof createClient>>) {
  const {
    data: { user },
  ...
  // Verify the organisation is approved
...
export async function POST(request: NextRequest) {
  ...
  const admin = await getOrgAdmin(supabase);
  if (!admin) {
  ...
  if (insertError || !newKey) {
    console.error("api_key insert error", insertError);
  ...
  await service.from("audit_log").insert({
    ...
    entity_id: newKey.id,
    before: null,
    after: { label, organisation_id: profile.organisation_id },
  });
...
export async function DELETE(request: NextRequest) {
  ...
  // Verify the key belongs to this organisation
  ...
  if (!existingKey || existingKey.organisation_id !== profile.organisation_id) {
